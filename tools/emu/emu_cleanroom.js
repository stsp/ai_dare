// Dump every room five times with Dan a few cells apart, so a cell-wise vote
// can drop him: node emu_cleanroom.js room_snaps.json OUTDIR
const { boot, OUT } = require('./emu_lib');
const fs = require('fs');
const KEYS = { P: [5, 1], O: [5, 2], Q: [2, 1] };
const [listFile, outDir] = process.argv.slice(2);
const list = JSON.parse(fs.readFileSync(listFile, 'utf8'));
(async () => {
  const E = await boot();
  await E.page.evaluate(() => {
    const w = window.__workers[0];
    window.__frames = (n) => new Promise((res) => { let left = n; const h = (e) => { if (e.data.message === 'frameCompleted') { if (--left <= 0) { w.removeEventListener('message', h); res(); } else w.postMessage({ message: 'runFrame', frameBuffer: new ArrayBuffer(26112) }); } }; w.addEventListener('message', h); w.postMessage({ message: 'runFrame', frameBuffer: new ArrayBuffer(26112) }); });
    window.__key = (row, mask, down) => w.postMessage({ message: down ? 'keyDown' : 'keyUp', row, mask });
    window.__step = async (keys, n) => { for (const [r, m] of keys) __key(r, m, true); await __frames(n); for (const [r, m] of keys) __key(r, m, false); const d = await __peek(0xC012, 2); const rm = await __peek(0x6297, 1); return { room: rm[0], y: d[0], x: d[1] }; };
  });
  const step = (keys, n) => E.page.evaluate(([k, n]) => __step(k, n), [keys.map((k) => KEYS[k]), n]);
  for (const [room, snap] of Object.entries(list)) {
    try {
      await E.loadFile(snap);
      let s = await step([], 160);                       // the entry message goes
      const moves = process.env.SLOW ? [[], ['P', 8], ['Q', 3], ['O', 20], ['Q', 3], ['P', 6]] : process.env.JUMPY ? [[], ['Q', 3], ['O', 5], ['Q', 3], ['P', 10], ['Q', 3]] : [[], ['P', 8], ['P', 8], ['O', 24], ['O', 8]];
      const settle = process.env.SLOW ? 50 : process.env.JUMPY ? 2 : 12;
      let got = 0;
      for (let i = 0; i < moves.length; i++) {
        if (moves[i].length) s = await step([moves[i][0]], moves[i][1]);
        s = await step([], settle);
        if (s.room !== +room) { console.log(`room ${room}: left to ${s.room} at dump ${i}`); break; }
        const dir = `${OUT}${outDir}/pos${i}`; fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(`${dir}/room_${room}.scr`, Buffer.from(await E.peek(0x4000, 6912)));
        got++;
      }
      console.log(`room ${room}: ${got} dumps, x ${s.x} y ${s.y}`);
    } catch (e) { console.log(`room ${room}: failed ${e.message}`); }
  }
  await E.browser.close();
})();
