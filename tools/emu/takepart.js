...
(async () => {
  ...
  const step = (n) => page.evaluate((n) => { for (let i = 0; i < n; i++) { updateAi(1 / 60); updatePickups(); updateGuards(1 / 60); updateLasers(1 / 60); if (state.partFlash > 0) state.partFlash -= 1 / 60; if (state.messageTimer > 0) state.messageTimer -= 1 / 60; if (state.viewerTimer > 0 && (state.viewerTimer -= 1 / 60) <= 0) state.viewer = 'asteroid'; if (state.viewerStatic > 0) state.viewerStatic -= 1 / 60; runCues(1 / 60); } draw(); return { x: ai.x, y: ai.y, carrying: state.carrying, flash: state.partFlash, msg: state.msgTop, low: state.msgBottom, viewer: state.viewer }; }, n);
