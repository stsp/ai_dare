...
(async () => {
  ...
  const info = await page.evaluate(() => {
    ...
    const r = ROOMS['255']; resetDan(120, 96); enterRoom('255', 120, 96); dan.invuln = 1e9; treens = [];
    return guns.map((g) => ({ type: g.type, x: g.x, y: g.y, w: g.w, h: g.h, cy: g.cy }));
  });
  console.log(JSON.stringify(info));
  let n = 0;
  for (let i = 0; i < 600 && n < 3; i++) {
    const st = await page.evaluate(() => { updateDan(1 / 60); updateGuns(1 / 60); draw(); return gunShots.filter((s) => !s.done && s.by.type === 0).map((s) => [s.x, s.y, s.dx, s.dy]); });
