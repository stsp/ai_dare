...
(async () => {
  ...
  const out = await page.evaluate(() => {
    ...
    for (let i = 0; i < 8 * 60; i++) {
      ...
      tick(); t += dt;
      trail.push({ t, x: ai.x, room: state.room, score: state.score, time: state.timeLeft, guards: guards.filter((x) => !x.dead).length, hist: history.length });
    }
    const before = trail[trail.length - 1];
    rewind(REWIND_SECS);
    const after = { x: ai.x, room: state.room, score: state.score, time: state.timeLeft, guards: guards.filter((x) => !x.dead).length, hist: history.length };
    ...
    rewind(REWIND_SECS);
    const after2 = { x: ai.x, score: state.score, time: state.timeLeft, hist: history.length };
