    ...
    for (const key of ROOM_IDS) {
      ...
      for (const p of r.platforms.filter((p) => p.x1 - p.x0 >= 5)) {
        ...
        for (let i = 0; i < 90 * 60; i++) {
          ...
          if (!killed) { const t = guards.find((t) => !t.dead && !t.riding); if (t) { killGuard(t); killed = true; } }
        }
        const seen = guards.filter((t) => !t.gone).length;   // one who ran out to come in where Ai is is the same guard
        most = Math.max(most, seen);
        if (seen > GUARD_MAX) bad.push(`${key} feet ${p.y * 8}: ${seen} guards came in all, one shot, ${guards.filter((t) => !t.dead).length} standing`);
      }
    }
