...
(async () => {
  ...
  const out = await page.evaluate(([room, x, y, key, frames]) => {
    const log = [];
    resetAi(+x, +y); enterRoom(room, +x, +y); ai.invuln = 1e9;
    for (let i = 0; i < 90; i++) updateAi(1 / 60);
    const st = () => `room ${state.room} x ${ai.x.toFixed(1)} y ${ai.y.toFixed(1)} g ${ai.onGround} lift ${ai.onLift ? (ai.onLift.dir + (ai.onLift.link ? '>' + ai.onLift.link.to : '') + (ai.onLift.shaft ? ' sh' : '')) : '-'}`;
    log.push('settled: ' + st());
    keys[key] = true;
    for (let i = 0; i < +frames; i++) { updateAi(1 / 60); if (i % 10 === 0) log.push(`${i}: ` + st()); }
    keys[key] = false;
    return log;
  }, [room, x, y, key, frames]);
