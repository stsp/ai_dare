...
(async () => {
  ...
  const info = await page.evaluate(([room, cell, pose, process_fitted]) => {
    ...
    const r = ROOMS[room]; const p = widestPlatform(r); resetDan(cell * 8, p.y - DAN_H); enterRoom(room, cell * 8, p.y - DAN_H); dan.invuln = 1e9;
    for (let i = 0; i < 30; i++) { updateDan(1 / 60); }
    if (pose === 'fire') { keys.Space = true; }
    if (pose === 'kneel') { keys.ArrowDown = true; }
    for (let i = 0; i < 3; i++) { updateDan(1 / 60); }
    if (typeof updateTreens === 'function') for (let i = 0; i < 3; i++) updateTreens(1 / 60);
    draw();
    return { room: state.room, x: dan.x, y: dan.y, treens: treens.map((t) => [Math.round(t.x), Math.round(t.y), t.dir, t.entering]) };
