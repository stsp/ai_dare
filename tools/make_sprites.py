#!/usr/bin/env python3
"""Build assets/ai.png from the Ai pose renders, which are kept out of the
repository (assets/ai.png is committed; put the renders beside this file to rebuild it).
...
are scaled so a running Ai stands AI_H screen cells tall, packed into one
sheet at 3x (the canvas scale, so the game draws them 1:1), and described in
js/ai_sheet.js: where each frame sits, and where his body is within it so the
...
def main():
    ...
    Image.fromarray(head, "RGBA").save("assets/ai_head.png", optimize=True)
    head_meta = {"image": "assets/ai_head.png", "scale": round(SCALE / scale, 3), "w": head.shape[1], "h": head.shape[0],
    ...
    Image.fromarray(sheet, "RGBA").save("assets/ai.png", optimize=True)
    meta = {"image": "assets/ai.png", "scale": SCALE, "frames": frames, "head": head_meta}
    with open("js/ai_sheet.js", "w") as f:
