...
def main():
    ...
    ap.add_argument("--solid", default="", help="the original's per-cell flag maps (data/emu/solid.json): the cells it "
                         "draws over Dan - walls, walkways, shafts, doorways - packed into the index a row at a time")
    ...
    if args.solid:
        for room, rows in json.load(open(args.solid)).items():
            if room in index:
                solid[room] = [sum(1 << (c - 1) for c in range(1, 31) if rows[r][c] == "#") for r in range(0, 18)]
        print(f"foreground maps for {len(solid)} rooms")
    ...
    with open(args.index, "w") as f:
        ...
        f.write("window.ROOMS_SHEET = " + json.dumps({"image": "assets/rooms.png", "w": 240, "h": 144, "rooms": index, "doors": door_index,
                                                     "guns": guns, "items": items, "buttons": buttons, "solid": solid, "arrows": arrows, "arrow": arrow_bits if args.arrow else None}) + ";\n")
